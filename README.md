...
- **반경 1km** (`RADIUS_METERS`). 도로 소통의 `ITS_HALF_SPAN`(약 555m)보다 넓습니다.
  ...
  있으면 됩니다. 실측에서 경포해수욕장의 가장 가까운 실시간 주차장이 약 960m 였습니다.
...
그래서 **점유가 경계값(0 또는 전체)인 채로 20분 이상 한 번도 변하지 않으면**
(`ParkingSensorGuard.STUCK_THRESHOLD`) 그 주차장만 실시간에서 내리고 이름·거리·총 주차면
까지만 남깁니다. 값을 고치지 않고 쓰지 않을 뿐입니다.

- 판정 기준 시각은 **캐시가 공급자를 실제로 부른 시각**입니다. 캐시가 적중한 요청은 새
  관측이 아니므로 연속 시간을 늘리지 않습니다.
- 경계값이 아닌 값은 아무리 오래 그대로여도 고착으로 보지 않습니다. 한산한 주차장은 실제로
  몇 시간 동안 값이 변하지 않습니다.
- 20분은 실시간 캐시 5분 기준 네 번 연속 같은 경계값을 본 셈입니다. 짧게 잡으면 실제로 텅 빈
  주차장이 자꾸 정보 없음이 되고, 길게 잡으면 죽은 센서를 그만큼 오래 내보내게 됩니다.
