 ...
 * 빠진 장소가 순위 미수록인지 파일이 깨진 것인지 구분할 수 없다.
 */
public final class TmapRankCsvParser {
    ...
    private TmapRankCsvParser() {
    }

    public static List<TmapRankCsvRow> parse(InputStream in, String fileName) {
        try (BufferedReader reader = new BufferedReader(
            ...
            if (headerLine == null) {
                throw new TmapRankImportException("빈 파일입니다: " + fileName);
            }
            ...
            throw new TmapRankImportException("파일을 읽지 못했습니다: " + fileName, e);
        }
    }
    ...
    private static List<TmapRankCsvRow> readRows(BufferedReader reader, String fileName)
            throws IOException {

        List<TmapRankCsvRow> rows = new ArrayList<>();
        ...
        if (rows.isEmpty()) {
            throw new TmapRankImportException("데이터 행이 없습니다: " + fileName);
        }

        return rows;
    }

    private static TmapRankCsvRow toRow(CSVRecord record, String fileName) {
        long line = record.getRecordNumber() + 1;

        return new TmapRankCsvRow(
