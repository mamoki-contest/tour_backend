...
public class AttractionController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    (카탈로그는 관광지명 오름차순, 공급자 폴백은 공급자 순서) 그대로입니다.
                    `sort` 는 적용 여부와 무관하게 **요청한 값을 그대로** 싣습니다.
                    순서에 뜻이 있는지는 `sort` 가 아니라 `sortApplied` 로 판단하세요.
                    언급량 스냅샷을 아직 수집하지 않은 환경에서 늘 이렇게 됩니다.

                    ### 정렬·지도 경계의 대상 집합
                    `sort` 나 지도 경계를 주면 **서버에 적재된 관광지 카탈로그 전수**가 대상입니다.
                    카탈로그는 온라인 언급량을 수집할 때 순회하는 바로 그 집합이라,
                    언급량을 산정한 집합과 정렬하는 집합이 항상 같습니다.
                    이 경우 `collectedAt` 은 **카탈로그를 마지막으로 적재한 시각**입니다.
                    적재를 다시 돌리면 공급자 내용이 그대로여도 이 시각은 앞으로 갑니다.
                    적재 이력이 한 줄도 없는 환경에서만 카탈로그 내용이 바뀐 시각을 씁니다.

                    카탈로그가 아직 적재되지 않은 환경에서는 공급자 응답을 모아 정렬합니다.
                    조건에 맞는 장소가 없으면 `dataStatus` 는 `AVAILABLE` 이고 `items` 만 빕니다.
                    `NO_DATA` 는 데이터를 얻지 못했다는 뜻이라 다릅니다.
                    ...
                    - `FIXED` + `visitDate`: 그 날이 **그 장소 자신의 지원 범위 안 분포**에서
                      한산(`LOW`)/보통(`NORMAL`)/혼잡(`HIGH`) 중 어디인지 반환합니다.
                    - `FLEXIBLE`: 그 장소의 지원 범위 중 한산 예상일(`quietestDate`)을 반환합니다.
                    ...
                    **서로 다른 관광지의 status 를 모아 혼잡도 순위로 쓰지 마세요.**
                    ...
                    지원 범위는 공급자 예측 창에서 나옵니다. 창의 기준일이 하루 뒤처지는 날이 있어
                    오늘부터 29일일 수도, 30일일 수도 있습니다. 범위를 고정으로 가정하지 말고
                    응답의 `supportedFrom` ~ `supportedTo` 를 그대로 표시하세요.
                    """)
    @ApiResponses({
     ...
     * 관광지 상세를 조회한다.
     *
     * <p>기본정보, 방문 혼잡도 예측, 대체지 후보, 함께 가기 좋은 곳을 각각 독립 필드로 돌려준다.
    ...
    @Operation(
            summary = "관광지 상세 조회",
            description = """
                    관광지 상세 정보를 반환합니다. 기본정보·방문 혼잡도 예측·대체지 후보·
                    ...
                    지원 범위의 하루치 판정을 담습니다. `dailyForecast` 의 길이는 공급자 창에 따라
                    29일 또는 30일이며 `supportedFrom` ~ `supportedTo` 와 일치합니다.
                    두 값 모두 **그 장소 자신의 지원 범위 안 분포에서의 상대 수준**입니다.
                    집중률 원본값은 담지 않습니다.
