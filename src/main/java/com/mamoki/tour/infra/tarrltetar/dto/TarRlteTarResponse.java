...
/** TarRlteTarService1 의 원본 응답 구조. 공공데이터포털 B551011 계열 공통 봉투를 따른다. */
...
public record TarRlteTarResponse(Response response) {
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record Items(List<TarRlteTarItem> item) {
    }

    public List<TarRlteTarItem> items() {
        if (response == null || response.body() == null || response.body().items() == null) {
            return List.of();
        }

        List<TarRlteTarItem> item = response.body().items().item();
